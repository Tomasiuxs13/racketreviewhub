...
### For New Rackets
- 🤖 **Ratings** - Estimated by ChatGPT based on racket characteristics (shape, brand, specs)
- 🤖 **Review** - Generated by ChatGPT automatically
- All Excel fields are imported
...
6. **ChatGPT Ratings**: For new rackets without ratings in Excel, ChatGPT estimates them based on:
   - Racket shape (diamond = power, round = control, teardrop = balanced)
   - Brand reputation
   - Specification fields (balance, core, game level, etc.)

7. **ChatGPT Reviews**: New rackets automatically get AI-generated reviews

8. **Year Default**: If year is missing, it defaults to the current year

9. **Existing Racket Updates**: Only `currentPrice` and `affiliateLink` are updated - all other fields (ratings, reviews, specs) are preserved
...
- `Title_URL` → Used for product URL and matching existing rackets
- `Current price` → Current price ✅ (always updated)
- `Affiliate Link` or `Link` → Affiliate link ✅ (updated if provided)
- `Previous price` → Original price (only for new rackets)
- `Brand` → Brand name ✅
- `Model` → Model name ✅
- `Shape` → Racket shape ✅ (helps ChatGPT estimate ratings)
- `Image_URL` → Image URL (only for new rackets)
- `Color`, `Balance`, `Surface`, `Hardness`, `Finish`, `Core`, `Game Level`, etc. → Specs (only for new rackets, helps ChatGPT)
